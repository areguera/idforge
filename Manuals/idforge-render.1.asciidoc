...
////
The *idforge-render(1)* command produces images, documents and
packages as specified in configuration files.
////

The *idforge-render(1)* command produces images and documents as
specified in configuration files.

== Options

The *idforge-render(1)* command accepts the following command-line
options:

include::Commons/options.inc[]

The *idforge-render(1)* command reads configuration from directories
...
The *idforge-render(1)* command processes configuration sections from
...
render-type::
    ...
    configuration option is used as default value.  Possible values to
    this option are the following:
+   
- "svg" -- Transforms source files in SVG format to PNG format in
  several ways using Inkscape.
- "png" -- Transforms PNG images in several ways using ImageMagick
  tools suite.
- "asciidoc" -- Transforms source files in asciidoc format to manpage, xhtml and
  pdf formats.
- "docbook" -- Transforms source files docbook format to manpage, xhtml and
  pdf formats.
...
The *idforge-render(1)* command controls content localization based on
...
The *idforge-render(1)* command controls the final output location
...
The *idforge-render(1)* command initiates image construction when
...
The *idforge-render(1)* command initiates document construction when
...
In the very specific case of processing modified docbook instances
with manpage document structure (i.e., render-flow = "manpage"), the
following configuration options are available for you to customize:

manpage-software::
    Specifies the name of the software product or component to which
    this topic applies. When this option is not specified in the
    configuration section, the value of IDFORGE environment variable
    is used as default value.
manpage-version::
    Specifies the version of the software product or component to
    which this topic applies. When this option is not specified in the
    configuration section, the value of IDFORGE_VERSION environment
    variable is used as default value.
manpage-sectdesc::
    Specifies the section title of the reference page. When this
    option is not specified in the configuration section, its value is
    determined automatically based on the manpage volume number set
    inside the modified docbook instance.

=== Compressing Files

The *idforge-render(1)* command initiates file compression when
...
The *idforge-render(1)* command initiates files archiving when
...
*idforge-render(1)* needs you to create a working directory using the
...
./Models/Brands/168/logo.gpl::
    ...
    color you have as result of indexing the PNG version of your
    project's logo to its minimum number of colors.
./Models/Brands/168/symbol.svg::
    The symbol design of your organization in SVG format, 168 pixels
    of height and the appropriate white space for content protection
    from borders.
./Models/Brands/168/symbol.png::
    The symbol design of your organization in PNG format, indexed to
    the minimum number of color possible. This image is normally
    produced from its SVG version and later indexed using GIMP.
./Models/Brands/168/symbol.gpl::
    The color information of your project symbol. This is the palette
    of color you have as result of indexing the PNG version of your
    project's symbol to its minimum number of colors.
./Fonts/::
    This directory contains your project's typographies. This is the
    font used to build your project's logotype and any other font your
    project is using in its graphic designs. Files in this directory
    must be in TrueType Font (ttf) format.
...
Asciidoc format, image sources in SVG format, configuration files used
to gear everything up, and descriptions to help you extend the basic
layout for your particular situation.
