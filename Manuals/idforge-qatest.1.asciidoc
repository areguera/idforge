...
*idforge-qatest* is the unit testing framework used by *idforge*
developers to assure quality of idforge and its sub-commands.

*idforge-qatest* is conceived to make easier for you to create and run
automated quality assurance tests for *idforge* and its sub-commands
quickly and consistently.  Normally, you run *idforge-qatest* several
times a day, before committing changes up to public repositories. So,
to be sure your local changes didn't break anything. Likewise, when
you fix a bug, to reproduce the expected behaviour and refrain it from
happening again.

When you execute *idforge-qatest* it looks for shell scripts inside
the first level of the directories you provided as argument in the
command-line and executes them, one by one, in the order they were
...
In *idforge-qatest*, the integrity checks are triggered whenever the
...
*idforge-qatest* makes a list of all files inside the directory where
...
either of such files are detected, *idforge-qatest* makes a list of
...
The exit status of *idforge-qatest* command execution is the number of
...
After the last test has been executed, *idforge-qatest* displays a
...
the report line, *idforge-qatest* concludes its execution suggesting
the action you should adopt, based on the results it found. 

Normally, when one test fails, *idforge-qatest* considers the entire
...
sake, *idforge-qatest* plays a significant role. It is responsible of
