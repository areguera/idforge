...
----------------------------------------------------------------------
=IDFORGE_COMMAND_USAGE=
----------------------------------------------------------------------

== Description

*idforge-qatest* is a unit testing framework to create and run
automated quality assurance tests for idforge automation scripts
quickly and consistently. The *idforge* developers, run
*idforge-qatest* several times a day. Normally, before committing
...
*-a, --add*::
    ...
    this option, you are prompted to enter the commands you want the
    new quality assurance test to have.
...
tests against the installed version, it just doesn't make too much
sense if the same tests were already applied and passed before
releasing the code that is installed. 
...
|-- qatest-2014121354413938903.sh           <4>
|-- qatest-2014121354413938903.sha1sum      <5>
`-- qatest-2014121354413938903.sha256sum    <6>
----------------------------------------------------------------------
...
<4> Shell script initiate integrity checks.
<5> Integrity check file, based on digests using sha1sum command.
<6> Integrity check file, based on digests using sha256sum command.
======================================================================
