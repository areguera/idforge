...
_idforge_ module environments are a collection of functions written in
Bash that can call themselves one another to create individual
execution environments. Module environments can be nested to achieve
high levels of maintainability and extensibility. 

_idforge_ module environments were introduced in tcar-0.5, making it
possible to divide complicated tasks into smaller tasks that can be
...
This page describes _idforge_ module environments.  To make this
description easier to understand, it uses examples taken from _idforge
--debug hello_ command. The description starts with a general
description about module environments for further delving into their
implementation using _idforge_ debugging output as reference.

[[module-environment]]
== Module Environment

When you execute _idforge_ you create a shell environment in which
variables and functions can be defined. This environment is the first
one defined in _idforge_ so it has the highest scope you can work
with. Inside this environment, we export variables and functions that
will always be available at run-time.

_idforge_ shell environment can be customized passing environment
variables described in idforge(1) through command-line. This is useful
to run _idforge_ in a location different from its default one (e.g.,
when quality assurance tests are run in the build root).

_idforge_ shell environment is small and doesn't provide too much
functionality beyond library initialization. A picture of idforge
shell execution environment is presented below:
...
_idforge_setModuleEnvironment_ function with the _-t "parent"_
arguments set on it. 

When the --debug option is provided to _idforge_, the parent modules
...
inside _idforge_ script.  Parent modules are frequently used to define
...
_idforge_ script to execute the function definition and destroy it
...
When you write functions for _idforge_ modules, you should add a top
...
_idforge_ provides four resources that you can use to optimize your
...
standard we use inside _idforge_ for parsing arguments passed through
...
executed, related functions are exported to _idforge_ shell
...
point, in one single iteration of _idforge_.  If you need to execute
...
_idforge_setModuleEnvironment_ function with the _-t "child"_
arguments set on it. 
...
_idforge_, they create a chain of modules.  Chain of modules are
...
documentation, but not both, in one single iteration of _idforge_
script).  
...
by executing the _idforge_setModuleEnvironment_ function with the _-t
...
cases like this, the _idforge_ script doesn't destroy the last module.
