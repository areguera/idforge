...
*render-from* and *render-from-po* options have been provided in it.
From this analysis the following two relations between files can be
...
Processing files in one-to-one relation::
    ...
    *idforge-locale* creates/updates translation files in one-to-one
...
Processing files in many-to-one relation::
    ...
    translation files provided, *idforge-locale* only updates the
...
*idforge-locale* accepts the following command-line options:
...
*idforge-locale* understands most options from *idforge-render(1)*
configuration files. Specially, *render-from*, *render-from-po* and
*render-from-mo*.
...
*idforge-locale* command to edit it. The PO file contains the
